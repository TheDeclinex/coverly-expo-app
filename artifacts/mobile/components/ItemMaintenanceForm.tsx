...
export const ItemMaintenanceForm = React.forwardRef<ItemMaintenanceFormHandle, { item: InventoryItem; rooms: Pick<InventoryRoom, "id" | "name" | "file_id">[]; onDirtyChange?: (dirty: boolean) => void; onSaveStateChange?: (state: ItemMaintenanceSaveState) => void }>(function ItemMaintenanceForm({ item, rooms, onDirtyChange, onSaveStateChange }, ref) {
  const colors = useColors();
  ...
  return <View style={styles.form}>
    ...
    <View style={[styles.section, styles.photosSection]}><View style={styles.photosHeading}><Feather name="image" size={15} color={colors.primary} /><Text style={[styles.photosTitle, { color: colors.foreground }]}>PHOTOS</Text></View><DraggablePhotoStrip photos={photos} onChange={(next) => { setPhotos(next); setPhotosModified(true); }} colors={colors} /></View>
