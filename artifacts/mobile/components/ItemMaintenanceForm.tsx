...
export const ItemMaintenanceForm = React.forwardRef<ItemMaintenanceFormHandle, {
  ...
  const replacementCurrencyToken = moneyDisplayToken(replacementCurrency, propertyMarket?.currency_code);
  const purchaseCurrencyToken = moneyDisplayToken(purchaseCurrency, propertyMarket?.currency_code);
  ...
  return <View style={styles.form}>
    ...
    <View style={styles.section}>
      ...
      <View style={[styles.valueCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <View style={styles.row}><View style={styles.grow}><Field label={`Each price (${replacementCurrencyToken})`}>{input("price")}</Field></View><View style={styles.qty}><Field label="Quantity"><QuantityStepper value={draft.quantity} onChange={(value) => set("quantity", value)} /></Field></View></View>
        <View style={[styles.valueSummary, { borderTopColor: colors.border }]}>
          <View><Text style={[styles.valueLabel, { color: colors.mutedForeground }]}>Recorded total</Text><Text style={[styles.valueTotal, { color: colors.foreground }]}>{recordedTotal === null ? "—" : formatCurrencyFull(recordedTotal, replacementCurrency, propertyMarket?.currency_code)}</Text></View>
