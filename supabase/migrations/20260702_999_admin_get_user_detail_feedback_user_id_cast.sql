-- Replace only public.admin_get_user_detail(uuid) to fix feedback_reports.user_id
-- text/uuid comparison in the live function body.
--
-- Required live change:
...
--
-- 1. Print the live lines around feedback_reports/fr.user_id:
-- WITH lines AS (
--   SELECT
--     row_number() OVER () AS line_no,
--     line
--   FROM regexp_split_to_table(
--     pg_get_functiondef('public.admin_get_user_detail(uuid)'::regprocedure),
--     E'\n'
--   ) AS line
-- )
-- SELECT line_no, line
-- FROM lines
-- WHERE line ILIKE '%feedback_reports%'
--    OR line ILIKE '%fr.user_id%'
--    OR line ILIKE '%p_user_id%'
-- ORDER BY line_no;
--
-- 2. Prove the broken comparison is gone and the fixed cast exists:
-- WITH lines AS (
--   SELECT btrim(line) AS line
--   FROM regexp_split_to_table(
--     pg_get_functiondef('public.admin_get_user_detail(uuid)'::regprocedure),
--     E'\n'
--   ) AS line
...
-- SELECT
--   EXISTS (
--     SELECT 1 FROM lines WHERE line = 'WHERE fr.user_id = p_user_id'
--   ) AS broken_comparison_present,
--   EXISTS (
--     SELECT 1 FROM lines WHERE line = 'WHERE fr.user_id = p_user_id::text'
--   ) AS fixed_comparison_present;
--
-- Expected:
--   broken_comparison_present = false
--   fixed_comparison_present = true
